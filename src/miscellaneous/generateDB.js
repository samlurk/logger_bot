...
async function generateTables() {
  ...
  loggerDB.on("error", (e) => {
    console.error(
      "There was an error while generating the database structure!",
      e
    );
  });
  try {
    const msgQuery = await loggerDB.query(
    ...
    ); // establish messages table
    const guildQuery = await loggerDB.query(
    ...
    ); // establish guilds table
    console.log(
      `DB generation completed, ${
        msgQuery.rowCount > 0 ? "generated" : "skipped"
      } messages table, ${
        guildQuery.rowCount > 0 ? "generated" : "skipped"
      } guilds table`
    );
  } catch (e) {
